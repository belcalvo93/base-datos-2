...
Cliente de línea de comandos: psql / Git Bash
...
```bash
pg_dump -U postgres -F c -f "respaldos/bd2_trabajo_AAAAMMDD.dump" bd2_trabajo
```
...
```bash
pg_restore -U postgres -d bd2_trabajo --clean "respaldos/bd2_trabajo_AAAAMMDD.dump"
```
...
2. Leer el script completo, línea por línea. Si una línea no se entiende, no se ejecuta.
3. Verificar que todo UPDATE y DELETE tenga WHERE, y que ese WHERE sea el correcto.
4. No confiar en el reporte del propio agente sobre lo que hizo: verificar en el motor.